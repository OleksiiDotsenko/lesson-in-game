...
```bash
ls "${LESSON_IN_GAME_HOME:-$HOME/lesson-in-game}/engine/node_modules" >/dev/null 2>&1 && echo READY || echo NEEDS_SETUP
```
...
From then on, use `ENGINE="${LESSON_IN_GAME_HOME:-$HOME/lesson-in-game}/engine"` for all commands.
Packs live in `${LESSON_IN_GAME_HOME:-$HOME/lesson-in-game}/packs/`.
...
```bash
mkdir -p "${LESSON_IN_GAME_HOME:-$HOME/lesson-in-game}/packs"
# write the pack to "${LESSON_IN_GAME_HOME:-$HOME/lesson-in-game}/packs/<packId>.json"
node "$ENGINE/validate-pack.js" "${LESSON_IN_GAME_HOME:-$HOME/lesson-in-game}/packs/<packId>.json"
```
