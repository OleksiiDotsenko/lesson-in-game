...
description: >-
  ...
  a Lesson in Game game — e.g. "show me the questions before we play", "let me fix question
...
Packs live in `${LESSON_IN_GAME_HOME:-$HOME/lesson-in-game}/packs/`. List them:
...
```bash
node "$ENGINE/validate-pack.js" "${LESSON_IN_GAME_HOME:-$HOME/lesson-in-game}/packs/<packId>.json" --launch
```
