...
Ludus turns an ordinary lesson, in any subject, into a live multiplayer game the
...
This copies the engine to `~/ludus/engine`, installs dependencies, and creates
the data home (`~/ludus/packs`, `sessions`, `reports`). Override the location
with `$LUDUS_HOME`. **Running a class needs no internet.**

### Run a game by hand (the skills do this for you)

```bash
ENGINE="$HOME/ludus/engine"
