'use strict';
/**
 * Data home: everything Ludus persists lives under ~/ludus (override: $LUDUS_HOME).
 * Local-first by design — student data never leaves this machine.
 *
 *   ~/ludus/packs/      content packs (draft + approved)
 *   ~/ludus/sessions/   one directory per session: log.jsonl, checkpoint.json, report.*
 *   ~/ludus/reports/    convenience copies of report.md for browsing
 */
...
function dataHome() {
  return process.env.LUDUS_HOME || path.join(os.homedir(), 'ludus');
}
